...
RUN mkdir -p $GOPATH/src/github.com/VidarSolutions
WORKDIR $GOPATH/src/github.com/VidarSolutions

RUN git clone -b $AVALANCHE_VERSION --single-branch https://github.com/VidarSolutions/avalanchego.git
...
RUN go mod edit -replace github.com/VidarSolutions/coreth=../coreth
...
COPY --from=builder /go/src/github.com/VidarSolutions/avalanchego/build .
