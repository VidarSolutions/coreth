...
import (
	...
	engCommon "github.com/VidarSolutions/avalanchego/snow/engine/common"
...
func createExportTxOptions(t *testing.T, vm *VM, issuer chan engCommon.Message, sharedMemory *atomic.Memory) []*Tx {
	...
	for _, addr := range testShortIDAddrs {
		exportTx, err := vm.newExportTx(vm.ctx.vidarAssetID, uint64(5000000), vm.ctx.XChainID, addr, initialBaseFee, []*secp256k1.PrivateKey{testKeys[0]})
		if err != nil {
			t.Fatal(err)
		}
...
func TestExportTxEVMStateTransfer(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:        "no transfers",
			tx:          nil,
			vidarBalance: big.NewInt(int64(vidarAmount) * x2cRateInt64),
			balances: map[ids.ID]*big.Int{
		...
		{
			name: "spend half vidar",
			tx: []EVMInput{
				{
					Address: ethAddr,
					Amount:  vidarAmount / 2,
					AssetID: testvidarAssetID,
					Nonce:   0,
				},
			},
			vidarBalance: big.NewInt(int64(vidarAmount/2) * x2cRateInt64),
			balances: map[ids.ID]*big.Int{
				customAssetID: big.NewInt(int64(customAmount)),
			},
			expectedNonce: 1,
		...
		{
			name: "spend too much vidar",
			tx: []EVMInput{
				{
					Address: ethAddr,
					Amount:  vidarAmount + 1,
					AssetID: testvidarAssetID,
					Nonce:   0,
				},
			},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			vidarUTXOBytes, err := vm.codec.Marshal(codecVersion, vidarUTXO)
			if err != nil {
				t.Fatal(err)
			}
			...
			vidarBalance := stateDB.GetBalance(ethAddr)
			if vidarBalance.Cmp(test.vidarBalance) != 0 {
				t.Fatalf("address balance %s equal %s not %s", addr.String(), vidarBalance, test.vidarBalance)
			}
...
func TestExportTxSemanticVerify(t *testing.T) {
	...
	validExportTx := &UnsignedExportTx{
		...
		ExportedOutputs: []*vidar.TransferableOutput{
			{
				Asset: vidar.Asset{ID: custom0AssetID},
				Out: &secp256k1fx.TransferOutput{
					Amt: custom0Balance,
					OutputOwners: secp256k1fx.OutputOwners{
						Threshold: 1,
	...
	validvidarExportTx := &UnsignedExportTx{
		NetworkID:        vm.ctx.NetworkID,
		BlockchainID:     vm.ctx.ChainID,
		DestinationChain: vm.ctx.XChainID,
		Ins: []EVMInput{
	...
	tests := []struct {
		...
		},
		{
			...
			tx: func() *Tx {
				validExportTx := *validvidarExportTx
		...
		},
		{
			...
			tx: func() *Tx {
				validExportTx := *validExportTx
				validExportTx.ExportedOutputs = []*vidar.TransferableOutput{{
					Asset: vidar.Asset{ID: custom0AssetID},
					Out: &secp256k1fx.TransferOutput{
						Amt: custom0Balance,
						OutputOwners: secp256k1fx.OutputOwners{
		...
		},
		{
			...
			tx: func() *Tx {
				validExportTx := *validExportTx
				exportOutputs := []*vidar.TransferableOutput{
					{
						Asset: vidar.Asset{ID: custom0AssetID},
						Out: &secp256k1fx.TransferOutput{
				...
				vidar.SortTransferableOutputs(exportOutputs, Codec)
				...
				validExportTx.ExportedOutputs = []*vidar.TransferableOutput{
					{
						Asset: vidar.Asset{ID: vm.ctx.vidarAssetID},
						Out: &secp256k1fx.TransferOutput{
							Amt: vidarBalance, // after fees this should be too much
							OutputOwners: secp256k1fx.OutputOwners{
								Threshold: 1,
								Addrs:     []ids.ShortID{addr},
							},
...
func TestExportTxAccept(t *testing.T) {
	...
	customUTXOBytes, err := Codec.Marshal(codecVersion, &vidar.UTXO{
		UTXOID: customUTXOID,
		Asset:  vidar.Asset{ID: custom0AssetID},
	...
	vidarUTXOBytes, err := Codec.Marshal(codecVersion, &vidar.UTXO{
		UTXOID: vidarUTXOID,
		Asset:  vidar.Asset{ID: vm.ctx.vidarAssetID},
	...
	if !bytes.Equal(fetchedValues[1], vidarUTXOBytes) {
		t.Fatalf("incorrect values returned expected %x got %x", vidarUTXOBytes, fetchedValues[1])
	}
}
...
func TestExportTxVerify(t *testing.T) {
	...
	vidar.SortTransferableOutputs(exportTx.ExportedOutputs, Codec)
	...
	tests := map[string]atomicTxVerifyTest{
		...
		"unsorted outputs": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *exportTx
				tx.ExportedOutputs = []*vidar.TransferableOutput{
		...
		"invalid exported output": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *exportTx
				tx.ExportedOutputs = []*vidar.TransferableOutput{tx.ExportedOutputs[0], nil}
				return &tx
			},
			ctx:         ctx,
			rules:       apricotRulesPhase0,
		...
		"non-vidar input Apricot Phase 6": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *exportTx
				tx.Ins = []EVMInput{
					{
		...
		"non-vidar output Apricot Phase 6": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *exportTx
				tx.ExportedOutputs = []*vidar.TransferableOutput{
					{
						Asset: vidar.Asset{ID: nonExistentID},
						Out: &secp256k1fx.TransferOutput{
							Amt: exportAmount,
							OutputOwners: secp256k1fx.OutputOwners{
								Locktime:  0,
		...
		"non-vidar input Banff": {
			...
			ctx:         ctx,
			rules:       banffRules,
			expectedErr: errExportNonvidarInputBanff.Error(),
		},
		"non-vidar output Banff": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *exportTx
				tx.ExportedOutputs = []*vidar.TransferableOutput{
					{
			...
			ctx:         ctx,
			rules:       banffRules,
			expectedErr: errExportNonvidarOutputBanff.Error(),
		},
	}
...
func TestNewExportTx(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			tx, err = vm.newExportTx(vm.ctx.vidarAssetID, exportAmount, vm.ctx.XChainID, testShortIDAddrs[0], initialBaseFee, []*secp256k1.PrivateKey{testKeys[0]})
			if err != nil {
				t.Fatal(err)
			}
			...
			burnedvidar, err := exportTx.Burned(vm.ctx.vidarAssetID)
			if err != nil {
				t.Fatal(err)
			}
			if burnedvidar != test.expectedBurnedvidar {
				t.Fatalf("burned wrong amount of vidar - expected %d burned %d", test.expectedBurnedvidar, burnedvidar)
			}
			...
			if sdb.GetBalance(addr).Cmp(new(big.Int).SetUint64(test.bal*units.vidar)) != 0 {
				t.Fatalf("address balance %s equal %s not %s", addr.String(), sdb.GetBalance(addr), new(big.Int).SetUint64(test.bal*units.vidar))
			}
		})
	}
}
...
func TestNewExportTxMulticoin(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			utxoID2 := vidar.UTXOID{TxID: ids.GenerateTestID()}
			utxo2 := &vidar.UTXO{
				UTXOID: utxoID2,
				Asset:  vidar.Asset{ID: tid},
				Out: &secp256k1fx.TransferOutput{
					Amt: importAmount2,
					OutputOwners: secp256k1fx.OutputOwners{
						Threshold: 1,
			...
			addr := GetEthAddress(testKeys[0])
			if stdb.GetBalance(addr).Cmp(new(big.Int).SetUint64(test.bal*units.vidar)) != 0 {
				t.Fatalf("address balance %s equal %s not %s", addr.String(), stdb.GetBalance(addr), new(big.Int).SetUint64(test.bal*units.vidar))
			}
