...
import (
	...
	avalanchegoMetrics "github.com/VidarSolutions/avalanchego/api/metrics"

	"github.com/VidarSolutions/coreth/consensus/dummy"
	corethConstants "github.com/VidarSolutions/coreth/constants"
	...
	"github.com/VidarSolutions/coreth/eth/ethconfig"
	"github.com/VidarSolutions/coreth/ethdb"
	corethPrometheus "github.com/VidarSolutions/coreth/metrics/prometheus"
	"github.com/VidarSolutions/coreth/miner"
	"github.com/VidarSolutions/coreth/node"
	"github.com/VidarSolutions/coreth/params"
	"github.com/VidarSolutions/coreth/peer"
	...
	"github.com/VidarSolutions/coreth/sync/client/stats"
	"github.com/VidarSolutions/coreth/sync/handlers"
	handlerstats "github.com/VidarSolutions/coreth/sync/handlers/stats"
	...
	// inside of cmd/geth.
	_ "github.com/VidarSolutions/coreth/eth/tracers/js"
	_ "github.com/VidarSolutions/coreth/eth/tracers/native"
	...
	"github.com/VidarSolutions/avalanchego/utils/perms"
	"github.com/VidarSolutions/avalanchego/utils/profiler"
	...
	commonEng "github.com/VidarSolutions/avalanchego/snow/engine/common"

	avalancheJSON "github.com/VidarSolutions/avalanchego/utils/json"
)
...
var (
	...
	// 1 nvidar and the smallest denomination on the C-Chain 1 wei. Where 1 nvidar = 1 gWei.
	// This is only required for vidar because the denomination of 1 vidar is 9 decimal
...
var (
	...
	errInsufficientFundsForFee        = errors.New("insufficient vidar funds to pay transaction fee")
...
func (vm *VM) Initialize(
	...
	vm.mempool = NewMempool(chainCtx.vidarAssetID, defaultMempoolSize)
...
func (vm *VM) preBatchOnFinalizeAndAssemble(header *types.Header, state *state.StateDB, txs []*types.Transaction) ([]byte, *big.Int, *big.Int, error) {
	for {
		...
		if rules.IsApricotPhase4 {
			contribution, gasUsed, err = tx.BlockFeeContribution(rules.IsApricotPhase5, vm.ctx.vidarAssetID, header.BaseFee)
			if err != nil {
...
func (vm *VM) postBatchOnFinalizeAndAssemble(header *types.Header, state *state.StateDB, txs []*types.Transaction) ([]byte, *big.Int, *big.Int, error) {
	...
	for {
		...
		// ApricotPhase5.
		txContribution, txGasUsed, err = tx.BlockFeeContribution(true, vm.ctx.vidarAssetID, header.BaseFee)
		if err != nil {
			return nil, nil, nil, err
		}
...
func (vm *VM) onExtraStateChange(block *types.Block, state *state.StateDB) (*big.Int, *big.Int, error) {
	...
	for _, tx := range txs {
		...
		if rules.IsApricotPhase4 {
			contribution, gasUsed, err := tx.BlockFeeContribution(rules.IsApricotPhase5, vm.ctx.vidarAssetID, block.BaseFee())
			if err != nil {
				return nil, nil, err
			}
...
func (vm *VM) CreateHandlers(context.Context) (map[string]*commonEng.HTTPHandler, error) {
	...
	vidarAPI, err := newHandler("vidar", &vidarAPI{vm})
	if err != nil {
		return nil, fmt.Errorf("failed to register service for vidar API due to %w", err)
	}
	enabledAPIs = append(enabledAPIs, "vidar")
	apis[vidarEndpoint] = vidarAPI
...
func (vm *VM) GetAtomicUTXOs(
	...
	startUTXOID ids.ID,
	limit int,
) ([]*vidar.UTXO, ids.ShortID, ids.ID, error) {
	...
	utxos := make([]*vidar.UTXO, len(allUTXOBytes))
...
func (vm *VM) GetSpendableFunds(
	...
	for _, key := range keys {
		...
		if assetID == vm.ctx.vidarAssetID {
			// If the asset is vidar, we divide by the x2cRate to convert back to the correct
			// denomination of vidar that can be exported.
...
// GetSpendablevidarWithFee returns a list of EVMInputs and keys (in corresponding
// order) to total [amount] + [fee] of [vidar] owned by [keys].
...
// corresponds to a single key, so that the signers can be passed in to
// [tx.Sign] which supports multiple keys on a single input.
func (vm *VM) GetSpendablevidarWithFee(
	keys []*secp256k1.PrivateKey,
	amount uint64,
	cost uint64,
	baseFee *big.Int,
	...
	for _, key := range keys {
		...
		addr := GetEthAddress(key)
		// Since the asset is vidar, we divide by the x2cRate to convert back to
		// the correct denomination of vidar that can be exported.
		balance := new(big.Int).Div(state.GetBalance(addr), x2cRate).Uint64()
