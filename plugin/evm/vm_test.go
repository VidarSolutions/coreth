...
import (
	...
	"github.com/VidarSolutions/coreth/internal/ethapi"
	...
	"github.com/VidarSolutions/coreth/core"
	"github.com/VidarSolutions/coreth/core/types"
	"github.com/VidarSolutions/coreth/eth"
	"github.com/VidarSolutions/coreth/params"
	"github.com/VidarSolutions/coreth/rpc"

	"github.com/VidarSolutions/coreth/accounts/abi"
	accountKeystore "github.com/VidarSolutions/coreth/accounts/keystore"
)
...
func NewContext() *snow.Context {
	...
	ctx.vidarAssetID = testvidarAssetID
...
func addUTXO(sharedMemory *atomic.Memory, ctx *snow.Context, txID ids.ID, index uint32, assetID ids.ID, amount uint64, addr ids.ShortID) (*vidar.UTXO, error) {
	utxo := &vidar.UTXO{
		UTXOID: vidar.UTXOID{
			TxID:        txID,
			OutputIndex: index,
...
// GenesisVMWithUTXOs creates a GenesisVM and generates UTXOs in the X-Chain Shared Memory containing vidar based on the [utxos] map
...
func GenesisVMWithUTXOs(t *testing.T, finishBootstrapping bool, genesisJSON string, configJSON string, upgradeJSON string, utxos map[ids.ShortID]uint64) (chan engCommon.Message, *VM, manager.Manager, *atomic.Memory, *engCommon.SenderTest) {
	...
	for addr, vidarAmount := range utxos {
		txID, err := ids.ToID(hashing.ComputeHash256(addr.Bytes()))
		if err != nil {
			t.Fatalf("Failed to generate txID from addr: %s", err)
		}
		if _, err := addUTXO(sharedMemory, vm.ctx, txID, 0, vm.ctx.vidarAssetID, vidarAmount, addr); err != nil {
...
func TestIssueAtomicTxs(t *testing.T) {
	...
	exportTx, err := vm.newExportTx(vm.ctx.vidarAssetID, importAmount-(2*params.AvalancheAtomicTxFee), vm.ctx.XChainID, testShortIDAddrs[0], initialBaseFee, []*secp256k1.PrivateKey{testKeys[0]})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReissueAtomicTxHigherGasPrice(t *testing.T) {
	...
	for name, issueTxs := range map[string]func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) (issued []*Tx, discarded []*Tx){
		"single UTXO override": func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) (issued []*Tx, evicted []*Tx) {
			utxo, err := addUTXO(sharedMemory, vm.ctx, ids.GenerateTestID(), 0, vm.ctx.vidarAssetID, units.vidar, testShortIDAddrs[0])
			if err != nil {
				t.Fatal(err)
			}
			tx1, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*vidar.UTXO{utxo})
			if err != nil {
				t.Fatal(err)
			}
			tx2, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(common.Big2, initialBaseFee), kc, []*vidar.UTXO{utxo})
			if err != nil {
				t.Fatal(err)
			}
		...
		"one of two UTXOs overrides": func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) (issued []*Tx, evicted []*Tx) {
			utxo1, err := addUTXO(sharedMemory, vm.ctx, ids.GenerateTestID(), 0, vm.ctx.vidarAssetID, units.vidar, testShortIDAddrs[0])
			if err != nil {
				t.Fatal(err)
			}
			utxo2, err := addUTXO(sharedMemory, vm.ctx, ids.GenerateTestID(), 0, vm.ctx.vidarAssetID, units.vidar, testShortIDAddrs[0])
			if err != nil {
				t.Fatal(err)
			}
			tx1, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*vidar.UTXO{utxo1, utxo2})
			if err != nil {
				t.Fatal(err)
			}
			tx2, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(common.Big2, initialBaseFee), kc, []*vidar.UTXO{utxo1})
			if err != nil {
				t.Fatal(err)
			}
		...
		"hola": func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) (issued []*Tx, evicted []*Tx) {
			...
			importTx1, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*vidar.UTXO{utxo1})
			if err != nil {
				t.Fatal(err)
			}

			importTx2, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(big.NewInt(3), initialBaseFee), kc, []*vidar.UTXO{utxo2})
			if err != nil {
				t.Fatal(err)
			}

			reissuanceTx1, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(big.NewInt(2), initialBaseFee), kc, []*vidar.UTXO{utxo1, utxo2})
			if err != nil {
				t.Fatal(err)
			}
			...
			reissuanceTx2, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(big.NewInt(4), initialBaseFee), kc, []*vidar.UTXO{utxo1, utxo2})
			if err != nil {
				t.Fatal(err)
			}
...
func TestBuildBlockDoesNotExceedAtomicGasLimit(t *testing.T) {
	...
	for i := 0; i < mempoolTxs; i++ {
		utxo, err := addUTXO(sharedMemory, vm.ctx, txID, uint32(i), vm.ctx.vidarAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)

		importTx, err := vm.newImportTxWithUTXOs(vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*vidar.UTXO{utxo})
		if err != nil {
			t.Fatal(err)
		}
...
func TestExtraStateChangeAtomicGasLimitExceeded(t *testing.T) {
	...
	// in ApricotPhase5.
	for i := 0; i < 100; i++ {
		_, err := addUTXO(sharedMemory1, vm1.ctx, txID, uint32(i), vm1.ctx.vidarAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)

		_, err = addUTXO(sharedMemory2, vm2.ctx, txID, uint32(i), vm2.ctx.vidarAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)
	}
